...
window.onload = ()=>{
    ...
    option.text = "No Pullbacks Found";
    document.getElementById("selectPullbacks").add(option);
}
...
function selectPullbackOnChange() {
    let x = 0;
    //TODO: Hide record button  
    //TODO: Send name to server
    //TODO: launch replay
    //TODO: Show replay slider
}
...
function requestFramePlayback(increment) {
    ...
    } else {//clearInterval(g_intervalPlaybackHandle);
    }
}
...
socket.onmessage = function(event) {
    if (event.data instanceof Blob) {
        ...
        case "frame":
            ...
            fileReader.onload = (event)=>{
                ...
                image.onload = ()=>{
                    ...
                    ctx.drawImage(offScreenCanvas, 0, 0, width, height, -c.width / 2, -c.height / 2, c.width, c.height);
                    ctx.restore();
            ...
            if (g_numberOfFrames == 1) {
                ...
                fileReader.onload = (event)=>{
                    ...
                    image.onload = ()=>{
                        ...
                        ctx.drawImage(offScreenCanvas, 0, 0, width, height, 0, 0, c.width, c.height);
        ...
        switch (lastMessage["response"]) {
        case "pullbacks":
            //Clear all select drop down items
            var selectMenu = document.getElementById("selectPullbacks");
            selectMenu.options.length = 0;
    
            var pullbackList = lastMessage["pullback"];
            for(var p in pullbackList){
                let option = document.createElement("option");
                option.text = pullbackList[p];
                document.getElementById("selectPullbacks").add(option);
            }
